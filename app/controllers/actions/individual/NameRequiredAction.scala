...
class NameRequiredAction @Inject()(val executionContext: ExecutionContext, val messagesApi: MessagesApi)
  extends ActionTransformer[DataRequest, ProtectorNameRequest] with I18nSupport {

  override protected def transform[A](request: DataRequest[A]): Future[ProtectorNameRequest[A]] = {
  ...
  private def getName[A](request: DataRequest[A]): String = {
    request.userAnswers.get(NamePage) match {
      ...
      case _ => request.messages(messagesApi)("individualProtector.name.default")
    }
  }
}
